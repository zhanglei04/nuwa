...
(function($) {
	$.fn
	.extend({ tableExport : function(options) {
		var defaults = {
			consoleLog : false,
			csvEnclosure : '"',
			csvSeparator : ',',
			...
			jspdf : {
				orientation : 'p',
				unit : 'pt',
				format : 'a4', // jspdf page format or 'bestfit' for autmatic paper
												// format selection
				...
				autotable : {
					styles : {
						...
						textColor : 50, // color value or 'inherit' to use css color from
														// html table
					...
					headerStyles : { fillColor : [
						52, 73, 94
					], textColor : 255, fontStyle : 'bold', halign : 'center' },
			...
			type : 'csv', // 'csv', 'txt', 'sql', 'json', 'xml', 'excel', 'doc', 'png'
										// or 'pdf'
		...
		if (defaults.type == 'csv' || defaults.type == 'txt') {
			...
			.each(function() {
				...
				if (jsonArrayTd.length > 0 && (jsonArrayTd.length != 1 || jsonArrayTd[0] != "")) jsonArray
				.push(jsonArrayTd);
				rowIndex++;
			});
			var jsonExportArray = [];
					...
					if (cell != null) {
						...
						for ( var styles in defaults.excelstyles) {
							if (defaults.excelstyles.hasOwnProperty(styles)) {
								trData += defaults.excelstyles[styles] + ': ' + $(cell)
								.css(defaults.excelstyles[styles]) + ';';
							}
						}
						if ($(cell).is("[colspan]")) trData += '" colspan="' + $(cell)
						.attr('colspan');
						if ($(cell).is("[rowspan]")) trData += '" rowspan="' + $(cell)
						.attr('rowspan');
			...
			if (defaults.jspdf.autotable === false) {
				...
				doc
				.addHTML($(el).first(), defaults.jspdf.margins.left, defaults.jspdf.margins.top, addHtmlOptions, function() {
					jsPdfOutput(doc);
				});
				// delete doc;
			} else {
				...
				// adjustment
				if (typeof defaults.jspdf.format === 'string' && defaults.jspdf.format
				.toLowerCase() === 'bestfit') {
					var pageFormats = { 'a0' : [
						2383.94, 3370.39
					], 'a1' : [
				...
				$(el)
				.filter(function() {
					return $(this).data("tableexport-display") != 'none' && ($(this)
					.is(':visible') || $(this).data("tableexport-display") == 'always');
				})
				.each(function() {
					var colKey;
					var rowIndex = 0;
					...
					if (typeof teOptions.onTable === 'function') if (teOptions
					.onTable($(this), defaults) === false) return true; // continue to
																															// next iteration
																															// step (table)
					// each table works with an own copy of AutoTable options
					defaults.jspdf.autotable.tableExport = null; // avoid deep recursion
																												// error
					...
					if (typeof atOptions.drawHeaderCell !== 'function') {
						atOptions.drawHeaderCell = function(cell, data) {
							...
							if (colopt.style.hasOwnProperty("hidden") != true || colopt.style.hidden !== true) return prepareAutoTableText(cell, data, colopt);
					...
					.each(function() {
						...
						ForEachVisibleCell(this, 'td', rowIndex, $hrows.length + $rows.length, function(cell, row, col) {
							if (typeof teOptions.columns[colKey] === 'undefined') {
								// jsPDF-Autotable needs columns. Thus define hidden ones for
								// tables without thead
								var obj = { title : '', key : colKey, style : { hidden : true } };
								teOptions.columns.push(obj);
							}
							if (cell !== null) {
								...
								var obj = $
								.extend(true, {}, teOptions.rowoptions[rowCount + ":" + (colKey - 1)]);
								obj.colspan = -1;
					...
					if (typeof teOptions.onBeforeAutotable === 'function') teOptions
					.onBeforeAutotable($(this), teOptions.columns, teOptions.rows, atOptions);
					teOptions.doc.autoTable(teOptions.columns, teOptions.rows, atOptions);
					// onAfterAutotable: optional callback function after returning
					// from jsPDF AutoTable that can be used to modify the AutoTable
					// options
					if (typeof teOptions.onAfterAutotable === 'function') teOptions
					.onAfterAutotable($(this), atOptions);
		...
		function ForEachVisibleCell(tableRow, selector, rowIndex, rowCount, cellcallback) {
			...
			.inArray(rowIndex - rowCount, defaults.ignoreRow) == -1) {
				...
				.filter(function() {
					return $(this).data("tableexport-display") != 'none' && ($(this)
					.is(':visible') || $(this).data("tableexport-display") == 'always' || $(this)
					.closest('table').data("tableexport-display") == 'always');
				}).find(selector);
				var rowColspan = 0;
				$row
				.each(function(colIndex) {
					if ($(this).data("tableexport-display") == 'always' || ($(this)
					.css('display') != 'none' && $(this).css('visibility') != 'hidden' && $(this)
					.data("tableexport-display") != 'none')) {
						...
						.inArray(colIndex - $row.length, defaults.ignoreColumn) == -1) {
							if (typeof (cellcallback) === "function") {
								...
								if ($(this).is("[rowspan]")) Rowspan = parseInt($(this)
								.attr('rowspan'));
		...
		function jsPdfOutput(doc) {
			...
			try {
				...
				downloadFile(defaults.fileName + '.pdf', 'data:application/pdf;base64,', doc
				.output());
			}
		}
		function prepareAutoTableText(cell, data, cellopt) {
			...
			if (cs >= 0) {
				...
				if (typeof cellopt != 'undefined' && cellopt.rowspan > 1) {
					if (cell.styles.valign === 'middle') cell.textPos.y = cell.textPos.y + (cell.height * (cellopt.rowspan - 1)) / 2;
					else if (cell.styles.valign === 'bottom') cell.textPos.y += (cellopt.rowspan - 1) * cell.height;
		...
		function csvString(cell, rowIndex, colIndex) {
			...
			if (cell != null) {
				...
				var csvValue = (dataString === null || dataString == '') ? '' : dataString
				.toString();
				if (dataString instanceof Date) result = defaults.csvEnclosure + dataString
				.toLocaleString() + defaults.csvEnclosure;
				else {
					result = replaceAll(csvValue, defaults.csvEnclosure, defaults.csvEnclosure + defaults.csvEnclosure);
					if (result.indexOf(defaults.csvSeparator) >= 0 || /[\r\n ]/g
					.test(result)) result = defaults.csvEnclosure + result + defaults.csvEnclosure;
				}
			}
			return result;
		}
		...
		function parseString(cell, rowIndex, colIndex) {
			...
			if (cell != null) {
				...
				if (typeof defaults.onCellHtmlData === 'function') htmlData = defaults
				...
				if (defaults.htmlContent === true) {
					...
					if (defaults.numbers.html.decimalMark != defaults.numbers.output.decimalMark || defaults.numbers.html.thousandsSeparator != defaults.numbers.output.thousandsSeparator) {
						...
						if (number !== false) {
							...
							result = (number < 0 ? "-" : "") + (defaults.numbers.output.thousandsSeparator ? ((mod ? frac[0]
							.substr(0, mod) + defaults.numbers.output.thousandsSeparator : "") + frac[0]
							.substr(mod)
							.replace(/(\d{3})(?=\d)/g, "$1" + defaults.numbers.output.thousandsSeparator)) : frac[0]) + (frac[1].length ? defaults.numbers.output.decimalMark + frac[1] : "");
						}
					}
				}
		...
		function getCellStyles(cell) {
			...
			return {
				style : {
					...
					color : rgb2array(getStyle(cell, 'color'), [
						0, 0, 0
					]),
					fstyle : f },
		...
		function getPropertyUnitValue(target, prop, unit) {
			...
			if (numeric !== null) {
				...
				target.parentElement.appendChild(temp); // insert it into the parent for
																								// em, ex and %
		...
		function downloadFile(filename, header, data) {
			...
			if (ua.indexOf("MSIE ") > 0 || !!ua.match(/Trident.*rv\:11\./)) {
				...
				if (DownloadLink) {
					...
					if (header.toLowerCase().indexOf("base64,") >= 0) DownloadLink.href = header + base64encode(data);
					...
					} else if (document.createEventObject) DownloadLink
					.fireEvent('onclick');
					else if (typeof DownloadLink.onclick == 'function') DownloadLink
					.onclick();
		...
		function base64encode(input) {
			...
			while (i < input.length) {
				...
				output = output + keyStr.charAt(enc1) + keyStr.charAt(enc2) + keyStr
				.charAt(enc3) + keyStr.charAt(enc4);
			}
			return output;
		}
		return this;
